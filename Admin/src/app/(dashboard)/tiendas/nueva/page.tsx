...
import { api, MerchantResponseDTO, StoreCategoryResponse } from '@/lib/api';
...
export default function NuevaTiendaPage() {
  ...
  useEffect(() => {
    api.categories.getAll().then(d => setCategories(d.filter(c=>c.active))).catch(()=>setCategories([]));
  }, []);
  ...
  return (
      <div className="space-y-12 animate-in slide-in-from-right duration-500 max-w-[1400px] mx-auto">
        <div>
          <button onClick={()=>router.back()} className="flex items-center gap-2 text-brand-camel font-bold text-[14px] mb-4 hover:underline">
        ...
        <Card className="p-10 space-y-10">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-x-12 gap-y-10">
              ...
              <div className="space-y-4">
                ...
                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-tight">Primordialmente para botones y títulos</p>
                <div className="space-y-3">
                  {PRIMARY_COLORS.map(c=>(
                      <button key={c.id} onClick={()=>setForm({...form,primaryColor:c.id})}
                              className={`w-full flex items-center justify-between p-3 rounded-[20px] border-2 transition-all ${form.primaryColor===c.id?'border-brand-black bg-white shadow-md':'border-neutral-100 hover:border-neutral-200'}`}>
                        ...
                        {form.primaryColor===c.id && <Check size={16} className="text-brand-black"/>}
                      </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-tight">Para acentos y elementos de soporte</p>
                <div className="space-y-3">
                  {SECONDARY_COLORS.map(c=>(
                      <button key={c.id} onClick={()=>setForm({...form,secondaryColor:c.id})}
                              className={`w-full flex items-center justify-between p-3 rounded-[20px] border-2 transition-all ${form.secondaryColor===c.id?'border-brand-black bg-white shadow-md':'border-neutral-100 hover:border-neutral-200'}`}>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full shadow-inner border border-neutral-100" style={{backgroundColor:c.code}}/>
                          <span className="text-[12px] font-extrabold text-neutral-900 tracking-wider uppercase">{c.name}</span>
                        </div>
                        {form.secondaryColor===c.id && <Check size={16} className="text-brand-black"/>}
                      </button>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <p className="text-[10px] font-bold text-neutral-400 uppercase tracking-tight">Detalles finos y estados de realce</p>
                <div className="space-y-3">
                  {TERTIARY_COLORS.map(c=>(
                      <button key={c.id} onClick={()=>setForm({...form,tertiaryColor:c.id})}
                              className={`w-full flex items-center justify-between p-3 rounded-[20px] border-2 transition-all ${form.tertiaryColor===c.id?'border-brand-black bg-white shadow-md':'border-neutral-100 hover:border-neutral-200'}`}>
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 rounded-full shadow-inner border border-neutral-100" style={{backgroundColor:c.code}}/>
                          <span className="text-[12px] font-extrabold text-neutral-900 tracking-wider uppercase">{c.name}</span>
                        </div>
                        {form.tertiaryColor===c.id && <Check size={16} className="text-brand-black"/>}
                      </button>
                  ))}
                </div>
              </div>
          ...
          <div className="flex justify-end gap-3 border-t border-neutral-100 pt-10">
            <Button variant="secondary" className="rounded-full px-10" onClick={()=>router.back()}>Cancelar</Button>
            <Button className="rounded-full px-10" onClick={handleSave} disabled={saving}>
              {saving ? <><Loader2 size={16} className="animate-spin mr-2"/>Guardando...</> : 'Guardar'}
            </Button>
          </div>
        </Card>
            ...
            <div className="fixed inset-0 bg-brand-black/20 backdrop-blur-md z-[100] flex items-center justify-center p-6">
              <motion.div initial={{opacity:0,scale:0.9,y:20}} animate={{opacity:1,scale:1,y:0}}
                ...
                <div className="p-10">
                  ...
                  <Input label="Buscar comerciante" placeholder="Nombre, apellido o correo..." icon={Search}
                         value={merchantSearch} onChange={e=>setMerchantSearch(e.target.value)}/>
                  <div className="max-h-[350px] overflow-y-auto space-y-2 pr-2 mt-4">
                    {loadingMerchants ? (
                        ...
                        <button key={m.id} onClick={()=>{setSelectedMerchant(m);setShowMerchantModal(false);setErrors(p=>({...p,merchant:''}));}}
