...
import { Plus, Search, X, Edit2, Trash2, Tag, AlertCircle } from 'lucide-react';
...
export function CategoriasScreen({ categories, stores, onCreate, onUpdate, onDelete }: CategoriasScreenProps) {
  ...
  const handleOpenEdit = (category: any) => {
    ...
    setFormData({
      ...
      assignedStores: category.assignedStores || []
    });
    setShowModal('edit');
  };
              ...
              <div className="p-10">
                ...
                <div className="mb-8">
                  <h4 className="text-[14px] font-bold text-brand-black mb-4 flex items-center gap-2">
                    <Tag size={18} className="text-brand-camel" />
                    Asignar a tiendas
                  </h4>
                  ...
                  <p className="text-[11px] text-neutral-400 mt-2 font-medium">
                    {formData.assignedStores.length} tienda(s) seleccionada(s)
                  </p>
                </div>

                <div className="flex gap-3 pt-4 border-t border-neutral-100">
                  <Button variant="secondary" className="flex-1 rounded-2xl h-14" onClick={handleCloseModal}>
                    Cancelar
                  </Button>
                  <Button
