  ...
  return (
    <div className="space-y-8 max-w-[1400px] mx-auto animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          <h2 className="text-[34px] font-display font-extrabold tracking-tight text-brand-black">Gestión de usuarios</h2>
        ...
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <Button onClick={() => setShowPanel('create')} className="flex-1 md:flex-none rounded-full h-12 px-6">
                ...
                {filteredUsers.map((user, i) => (
                  ...
                  >
                    ...
                    <td className="py-5 px-4">
                      <Badge variant={user.role === 'Administrador' ? 'active' : 'info'}>
                        {user.role}
                      </Badge>
                    </td>
          ...
          {showPanel && (
            ...
            >
              <Card className="p-8 border-2 border-neutral-100">
                <h3 className="text-[24px] font-display font-extrabold mb-6 text-brand-black">
                  {showPanel === 'create' ? 'Crear usuario' : 'Editar usuario'}
                </h3>
                
                <div className="space-y-5">
                  <Input label="Nombre completo" defaultValue={showPanel === 'edit' ? 'Lucía Vega' : ''} />
                  <Input label="Correo" defaultValue={showPanel === 'edit' ? 'lucia@street.com' : ''} />
                  <Select label="Rol" defaultValue={showPanel === 'edit' ? 'Comerciante' : undefined}>
                    <option>Administrador</option>
                    <option>Comerciante</option>
                    <option>Cliente</option>
                  </Select>
                  <Select label="Tienda asociada" defaultValue={showPanel === 'edit' ? 'Street/Core' : 'No aplica'}>
                  ...
                  <div className="flex gap-3 pt-4">
                    <Button variant="secondary" className="flex-1" onClick={() => setShowPanel(null)}>Cancelar</Button>
                    <Button className="flex-1">Guardar usuario</Button>
                  </div>
                </div>
              </Card>
            </motion.div>
