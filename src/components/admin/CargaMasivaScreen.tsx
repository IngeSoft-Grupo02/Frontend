...
// useApp ya no se usa para validación — los datos vienen del backend
...
const STORE_COLUMNS = ['storeName','slug','colorPalette','description','merchantEmail','logoFileName'];
const VALID_PALETTES  = ['CORESTREET','ATELIERMONO','UTILITYDROP','LUXECAPSULE'];
...
async function validateStoresCsv(
  ...
  rows.forEach((row, idx) => {
    ...
    // colorPalette
    if (!VALID_PALETTES.includes((row.colorPalette ?? '').toUpperCase()))
      incidences.push({ block:'Tiendas', row:n,
        detail:`colorPalette inválido: "${row.colorPalette}". Permitidos: ${VALID_PALETTES.join(', ')}.`, isError:true });
...
function downloadStoresTemplate() {
  const csv = [
    'storeName,slug,colorPalette,description,merchantEmail,logoFileName',
    ...
    'Mi Tienda Urbana,mi-tienda-urbana,CORESTREET,Ropa urbana para jovenes,juan.perez@ejemplo.com,MiTiendaUrbana.png',
    'Luxe Moda,luxe-moda,LUXECAPSULE,Alta costura accesible,maria.torres@ejemplo.com,',
  ].join('\n');
...
async function fetchExistingEmails(): Promise<string[]> {
  try {
    const res = await fetch(`${API_BASE}/admin/bulk/existing-emails`, {
...
async function fetchExistingStores(): Promise<{ storeNames: string[]; merchantEmails: string[] }> {
  try {
    const res = await fetch(`${API_BASE}/admin/bulk/existing-stores`, {
      headers: { ...getAuthHeader() },
    });
    if (!res.ok) return { storeNames: [], merchantEmails: [] };
    return await res.json();
  } catch { return { storeNames: [], merchantEmails: [] }; }
}
...
export function CargaMasivaScreen() {
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      if (blocks.images.file)    fd.append('logos',     blocks.images.file);

      const token = typeof window !== 'undefined' ? localStorage.getItem('token') : null;
      const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8080'}/admin/bulk/upload`,
          {
            method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(msg || `Error ${res.status} del servidor`);
      }
  ...
  const BLOCK_CONFIG = [
    ...
    { key:'stores' as BlockKey, label:'Tiendas',
      description:'Crea tiendas y las vincula a comerciantes. El merchantEmail es obligatorio y debe existir.',
