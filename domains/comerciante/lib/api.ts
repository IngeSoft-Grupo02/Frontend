...
const mapOrderItemDetail = (item: JsonValue) => ({
  ...
  physicalStock: (item.physicalStock ?? null) as number | null,
  reservedStock: (item.reservedStock ?? null) as number | null,
  stockShortage: (item.stockShortage ?? null) as number | null,
...
export const mapQuote = (raw: JsonValue): Quote => ({
  ...
  items: (raw.items || []).map((item: JsonValue) => ({
    ...
    physicalStock: item.physicalStock == null ? null : Number(item.physicalStock),
    reservedStock: item.reservedStock == null ? null : Number(item.reservedStock),
    stockShortage: item.stockShortage == null ? null : Number(item.stockShortage),
    productId: item.productId != null ? String(item.productId) : undefined,
