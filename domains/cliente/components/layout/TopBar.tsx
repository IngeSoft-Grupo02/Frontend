...
export const TopBar: React.FC<TopBarProps> = ({ store, user, onNavigate, onLogout, showSearch = true, cartCount = 0, currentView }) => {
  ...
  const storeIdentity = store && (
    <div className="min-w-0 flex items-center gap-3">
      <StoreLogo
        ...
        objectFit="contain"
      />
      <span className="max-w-[150px] truncate font-black text-[15px]" style={{ color: headerText }}>
        {store.name}
      </span>
    </div>
  );
  ...
  const accountActions = currentView !== View.DIRECTORY && (
    <>
      <button
  ...
  return (
    <header className="border-b sticky top-0 z-[100] transition-colors duration-300" style={{ backgroundColor: headerBg, color: headerText, borderColor: headerBorder }}>
      <div className="hidden min-h-[80px] items-center justify-between px-10 lg:flex">
        <div className="flex min-w-0 items-center">
          <div className="pr-6">{brand}</div>

          {store && (
            <div className="flex min-w-0 items-center gap-8 border-l pl-6" style={{ borderColor: isDirectory ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)' }}>
              {storeIdentity}
        ...
        <div className="flex shrink-0 items-center gap-8">{accountActions}</div>
      </div>
        ...
        {store ? (
          <>
            <div className="flex items-center justify-between gap-3">
              {storeIdentity}
              <div className="flex shrink-0 items-center gap-3">{accountActions}</div>
            </div>
            <nav className="grid grid-cols-4 items-center gap-2">{navLinks}</nav>
          </>
        ) : (
