...
import { Discount, Quote } from '@/domains/comerciante/lib/types';
...
export default function QuotesPage() {
  ...
  const totalRequestedQuantity = useMemo(() => {
    ...
    return selectedQuote.items.reduce((sum, item) => sum + item.quantity, 0);
  }, [selectedQuote]);
  ...
  const eligibleDiscounts = useMemo(() => {
    if (!selectedQuote || !discounts) return [];

    return discounts.filter((discount: Discount) => {
      if (discount.status !== 'Activa') return false;

      if (discount.appliesTo === 'Todo el catálogo') {
        return totalRequestedQuantity >= discount.minUnits;
      }

      // Producto específico: debe existir ese producto en la cotización y cumplir mínimo
      const matchingItem = selectedQuote.items.find(
          (item) => item.productId === discount.productId
      );
      return Boolean(matchingItem) && matchingItem!.quantity >= discount.minUnits;
    });
  }, [discounts, selectedQuote, totalRequestedQuantity]);
  ...
  const selectedDiscount = useMemo(
      () => eligibleDiscounts.find((d) => d.id === selectedDiscountId) || null,
      [eligibleDiscounts, selectedDiscountId]
  );
                    ...
                    <div className="p-6 space-y-10">
                        ...
                        <div className="mt-5 flex justify-end">
                          <div className="w-full md:w-80 space-y-2.5">
                            <div className="flex justify-between items-center text-[12px] font-bold text-brand-text-muted uppercase tracking-wider whitespace-nowrap gap-3">
                              ...
                              <span>S/ {pricing.base.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                            </div>
                            ...
                            {pricing.discountAmount > 0 && (
                                <div className="flex justify-between items-center text-[12px] font-bold text-red-500 uppercase tracking-wider whitespace-nowrap gap-3">
                                  <span>Descuento Aplicado</span>
                                  <span>- S/ {pricing.discountAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                                </div>
                            )}
                            <div className="pt-3 border-t-2 border-brand-neutral-border flex justify-between items-center gap-3">
                              <span className="text-[13px] font-black text-brand-black uppercase tracking-wider">Subtotal</span>
                              <span className="text-[18px] font-black text-brand-black tracking-tight whitespace-nowrap">
                            S/ {pricing.finalSubtotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                          </span>
                            </div>
                          </div>
                        </div>
                      ...
                      <div className="bg-brand-black rounded-[24px] p-6 flex flex-col md:flex-row items-center justify-between gap-5 text-white shadow-xl shadow-brand-black/30 border border-white/10">
                        <div className="space-y-1 w-full md:w-auto">
                          ...
                          <div className="flex flex-col gap-1 text-[10px] font-black text-white/70 uppercase tracking-[0.15em] pt-3">
                            <span className="text-white/70">Base de cotización: S/ {pricing.subtotal.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                            {pricing.discountAmount > 0 && (
                                <span className="text-red-400">Descuento aplicado: - S/ {pricing.discountAmount.toLocaleString('en-US', { minimumFractionDigits: 2 })}</span>
                            )}
                          </div>
                        </div>
                            ...
                            <div className="flex flex-col gap-4 w-full md:w-auto md:items-end">
                              ...
                              {eligibleDiscounts.length > 0 && (
                                  <div className="w-full md:w-64">
                                    <label className="text-[10px] font-black text-white/50 uppercase tracking-[0.2em] block mb-2 md:text-right">
                                      Descuento Disponible
                                    </label>
                                    <select
                                        value={selectedDiscountId || ''}
                                        onChange={(e) => setSelectedDiscountId(e.target.value || null)}
                                        className="w-full bg-transparent text-white border-2 border-white/20 rounded-xl px-3 py-2.5 text-[12px] font-bold outline-none focus:border-brand-camel transition-colors cursor-pointer appearance-none"
                                        style={{
                                    ...
                                    >
                                      <option value="" className="text-brand-black">Sin descuento</option>
                                      {eligibleDiscounts.map((d: any) => (
                              ...
                              <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
                                ...
                                <Button
                                    ...
                                    variant="camel"
                                    title={hasInsufficientStock ? 'No hay stock suficiente para aprobar esta cotización' : undefined}
                                    className={`h-12 px-8 gap-2 !rounded-xl font-black text-[13px] uppercase tracking-widest shadow-lg shadow-brand-camel/20 transition-all ${
                                        hasInsufficientStock
