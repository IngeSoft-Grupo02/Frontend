    ...
    <MerchantLayout title="Panel" subtitle={`Resumen de actividad · ${store.name}`}>
      <div className="flex flex-col gap-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <StatCard title="Pedidos Pendientes" value={stats.pendingOrders.toString().padStart(2, '0')} icon={ShoppingBag} onClick={() => router.push('/orders')} />
          <StatCard title="Cotizaciones Pendientes" value={stats.pendingQuotes.toString().padStart(2, '0')} icon={FileText} onClick={() => router.push('/quotes')} />
          <StatCard title="Borradores Pendientes" value={stats.drafts.toString().padStart(2, '0')} icon={Package} onClick={() => router.push('/products')} />
        </div>
